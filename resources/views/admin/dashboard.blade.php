...
@section('content')
    ...
    {{-- THỐNG KÊ TỔNG QUAN --}}
    <div class="row">
        @foreach ([['label' => 'Đơn hàng', 'value' => $totalOrders, 'icon' => 'fas fa-receipt', 'color' => 'info', 'route' => 'admin.orders.index'], ['label' => 'Doanh thu', 'value' => number_format($totalRevenue) . '₫', 'icon' => 'fas fa-coins', 'color' => 'success', 'route' => 'admin.orders.index'], ['label' => 'Tài khoản người dùng', 'value' => $totalUsers, 'icon' => 'fas fa-users', 'color' => 'primary', 'route' => 'admin.accounts.index'], ['label' => 'Sản phẩm', 'value' => $totalProducts, 'icon' => 'fas fa-book', 'color' => 'warning', 'route' => 'admin.books.index'], ['label' => 'Sách đã bán', 'value' => $totalBooksSold, 'icon' => 'fas fa-book-open', 'color' => 'dark', 'route' => 'admin.books.index'], ['label' => 'Đánh giá', 'value' => $totalReviews, 'icon' => 'fas fa-star', 'color' => 'secondary', 'route' => 'admin.reviews.index']] as $item)
    ...
    {{-- Nút chọn biểu đồ --}}
    <div class="mb-4">
        <button class="btn btn-outline-primary m-1" onclick="showChart('dailyRevenueCard')"><i
                class="fas fa-calendar-day"></i> Doanh thu ngày</button>
        <button class="btn btn-outline-info m-1" onclick="showChart('monthlyRevenueCard')"><i class="fas fa-chart-bar"></i>
            Doanh thu tháng</button>
        <button class="btn btn-outline-success m-1" onclick="showChart('yearlyRevenueCard')"><i
                class="fas fa-calendar-alt"></i> Doanh thu năm</button>
        <button class="btn btn-outline-dark m-1" onclick="showChart('dailyOrderCard')"><i class="fas fa-box"></i> Đơn hàng
            ngày</button>
        <button class="btn btn-outline-secondary m-1" onclick="showChart('monthlyOrderCard')"><i
                class="fas fa-th-large"></i> Đơn hàng tháng</button>
        <button class="btn btn-outline-warning m-1" onclick="showChart('yearlyOrderCard')"><i class="fas fa-handshake"></i>
            Đơn hàng năm</button>
        <button class="btn btn-outline-indigo m-1" onclick="showChart('monthlyUserCard')"><i class="fas fa-user-plus"></i>
            Người dùng mới</button>
        <button class="btn btn-outline-orange m-1" onclick="showChart('booksSoldCard')"><i class="fas fa-book"></i> Sách đã
            bán</button>
        <button class="btn btn-outline-purple m-1" onclick="showChart('booksCreatedCard')"><i
                class="fas fa-plus-square"></i> Sách mới thêm</button>
        <button class="btn btn-outline-brown m-1" onclick="showChart('booksImportedCard')"><i
                class="fas fa-truck-loading"></i> Sách nhập kho</button>
    </div>
    ...
    <div class="row flex-wrap">
        ...
        @foreach ([
            ...
            <div class="equal-height" id="{{ $id }}" style="display: {{ $loop->first ? 'block' : 'none' }};">
                <x-adminlte-card title="{{ $cfg['title'] }}" theme="{{ $cfg['theme'] }}"
                    icon="fas fa-{{ $cfg['icon'] }}">
...
@push('js')
    ...
    <script>
        const chartConfigs = [{
            ...
            {
                id: 'yearlyRevenueChart',
                labels: @json($yearLabels),
                data: @json($yearlyRevenueData),
                label: 'Doanh thu năm',
                type: 'bar',
                color: '#28a745'
            },
            ...
            },
            {
                ...
                labels: @json($monthlyOrderLabels),
            ...
            {
                id: 'booksSoldChart',
                labels: @json($booksSoldLabels),
                data: @json($booksSoldData),
                label: 'Sách đã bán',
                type: 'bar',
                color: '#fd7e14'
            },
            {
                id: 'booksCreatedChart',
                labels: @json($booksCreatedLabels),
                data: @json($booksCreatedData),
                label: 'Sách thêm mới',
                type: 'bar',
                color: '#6f42c1'
            },
            {
                id: 'booksImportedChart',
                labels: @json($booksImportedLabels),
                data: @json($booksImportedData),
                label: 'Sách nhập kho',
                type: 'bar',
                color: '#795548'
            },
